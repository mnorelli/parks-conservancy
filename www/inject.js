(function(exports) {
   ...
   */
  var qs={decode:function(str){str=String(str).replace(/\+/g,"%20");return decodeURIComponent(str)},encode:function(str){return encodeURIComponent(str).replace(/%2C/g,",").replace(/%3A/g,":").replace(/%3B/g,";").replace(/%20/g,"+")},parse:function(str){if(str.charAt(0)==="#"||str.charAt(0)==="?"){str=str.substr(1)}var data={},bits=str.split("&"),len=bits.length;for(var i=0;i<len;i++){var bit=bits[i];if(!bit)continue;var parts=bit.split("=",2),key=qs.decode(parts[0]);if(parts.length===1){val=true}else{var num=+val;if(isNaN(num)){switch(val){case"true":val=true;break;case"false":val=false;break}}else{val=num}}if(data.hasOwnProperty(key)){if(Array.isArray(data[key])){data[key].push(val)}else{data[key]=[data[key],val]}}else{data[key]=val}}return data},format:function(data){var keys=Object.keys(data),len=keys.length,bits=[];for(var i=0;i<len;i++){var k=keys[i];if(k&&data[k]!==null||typeof data[k]!=="undefined"){bits.push([qs.encode(k),qs.encode(String(data[k]))].join("="))}}return bits.join("&")}};
