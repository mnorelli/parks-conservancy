...
if(process.env.DYNO){
    CONN = env.require("DATABASE_URL");
}else{
    var config = url.parse(env.require("DATABASE_URL"));
...
var runQuery = lib.runQuery = function(query, params, callback){
    pg.connect(CONN, function(err, client, done) {
        if(err) {
            console.warn("error fetching client from pool:", err);
            return callback(err);
        }
        ...
        client.query(query, params, function(err, result) {
            ...
            if(err) {
                console.warn("error running query:", err);
                return callback(err);
            }
...
var processResult = lib.processResult = function(kind, result){
    var out = {};
    out.results = [];
